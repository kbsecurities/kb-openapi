...
- 각 TR의 실제 조회조건(dataBody) 필드는 KB 명세 원본(`json/*_B2C.json`)의
  `inputArray`에서 `inputFilter`에 나열된 필드(내부적으로 제외되는 필드)를 뺀
  나머지 기준으로 구현했습니다. `IVU10280`(거래량상위)부터 `IVU10550`(신고/신저)
  까지의 '상위/랭킹' 계열 TR은 겉보기엔 자본금·가격대·시가총액·거래량·액면가
  범위 필터 필드를 갖고 있지만, 명세상 전부 `inputFilter`에 포함되어 있어 실제
  요청에는 보내지 않습니다.
- `SIAM4983`(종목관리)은 명세상 입력 필드가 전부 제외 대상이라, 조회조건 없이
  (`dataBody={}`) 호출하면 전체 종목 마스터 정보를 반환합니다. 90여 개의 출력
  필드는 `frontend/src/app/openapi-test/samples.generated.json`의
  `Tkb_SIAM4983_B2C` 항목에서 확인할 수 있습니다.
